...
T NumericArray<T>::Max() const {
    if (repr_.Empty()) {
        // Handle undefined behavior for empty array
        throw std::runtime_error("Empty array");
    }
    T max = repr_[0];
    for (size_t i = 1; i < repr_.Size(); i++) {
        if (repr_[i] > max) {
            max = repr_[i];
        }
    }
...
T NumericArray<T>::Average() const {
    ...
    T sum = Sum();
    return sum / static_cast<T>(repr_.Size());
}
...
NumericArray<T>& NumericArray<T>::Sort() {
    for (size_t i = 0; i < repr_.Size(); i++) {
        for (size_t j = i + 1; j < repr_.Size(); j++) {
            if (repr_[i] > repr_[j]) {
                std::swap(repr_[i], repr_[j]);
            }
        }
    }
    return *this;
...
void NumericArray<T>::TrimZeroes() {
    ...
    size_t head = 0;
    size_t tail = repr_.Size() - 1;
    while (head < repr_.Size() && repr_[head] == 0) {
        head++;
    }
    while (tail > 0 && repr_[tail] == 0) {
        tail--;
    }
    if (head > tail) {
        repr_ = Vector<T>{};
        ...
        for (size_t i = head; i <= tail; i++) {
            result.Insert(i - head, repr_[i]);
        }
        repr_ = std::move(result.repr_);
    }
}

// Multiplication (*) operator (array * value)
template<typename T>
NumericArray<T> operator*(const NumericArray<T>& lhs, const T& rhs) {
    NumericArray<T> result;
...
// Multiplication (*) operator (value * array)
template<typename T>
NumericArray<T> operator*(const T& lhs, const NumericArray<T>& rhs) {
    return rhs * lhs;
}

// Division (/) operator (array / value)
template<typename T>
NumericArray<T> operator/(const NumericArray<T>& lhs, const T& rhs) {
    if (rhs == T{}) {
        // Handle undefined behavior for division by zero
