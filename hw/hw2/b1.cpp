...
bool operator==(const Set<T>& lhs, const Set<T>& rhs) {
    return lhs.Size() == rhs.Size() && std::all_of(lhs.begin(), lhs.end(), [&rhs](const T& val) {
...
template<typename T>
bool operator<=(const Set<T>& lhs, const Set<T>& rhs) {
    return std::all_of(lhs.begin(), lhs.end(), [&rhs](const T& val) {
        return rhs.Contains(val);
    });
}
...
bool Set<T>::IsDisjointWith(const Set<T>& other) const {
    return std::none_of(begin(), end(), [&other](const T& val) {
