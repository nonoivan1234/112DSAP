...
ImmutableSet<T>::ImmutableSet(std::initializer_list<T> l) : repr_(l) {
    std::sort(std::begin(repr_), std::end(repr_));
    repr_.erase(std::unique(std::begin(repr_), std::end(repr_)), std::end(repr_));
}

template<typename T>
ImmutableSet<T>::ImmutableSet(const std::vector<T>& values) : repr_(values) {
    std::sort(std::begin(repr_), std::end(repr_));
    repr_.erase(std::unique(std::begin(repr_), std::end(repr_)), std::end(repr_));
}
...
bool ImmutableSet<T>::Contains(const T& value) const {
    return std::binary_search(std::begin(repr_), std::end(repr_), value);
}

template<typename T>
bool ImmutableSet<T>::IsDisjointWith(const ImmutableSet<T>& other) const {
    return std::none_of(std::begin(repr_), std::end(repr_), [&other](const T& value) {
...
template<typename T>
bool operator<=(const ImmutableSet<T>& lhs, const ImmutableSet<T>& rhs) {
    return std::all_of(std::begin(lhs), std::end(lhs), [&rhs](const T& value) {
...
ImmutableSet<T> operator|(const ImmutableSet<T>& lhs, const ImmutableSet<T>& rhs) {
    std::vector<T> result;
    std::set_union(std::begin(lhs), std::end(lhs), std::begin(rhs), std::end(rhs), std::back_inserter(result));
    return ImmutableSet<T>(result);
}