...
FixedPoint::FixedPoint(double val) : repr_(std::llround(val * kInverseOfScalingFactor)) {}

FixedPoint::FixedPoint(long long repr) : repr_(repr) {}
...
FixedPoint FixedPoint::operator*(const FixedPoint& rhs) const {
    return FixedPoint(std::llround(repr_ * rhs.repr_ / (double)kInverseOfScalingFactor));
}

FixedPoint FixedPoint::operator/(const FixedPoint& rhs) const {
    return FixedPoint(std::llround(repr_ * (double)kInverseOfScalingFactor / rhs.repr_));
}
...
std::ostream& operator<<(std::ostream& os, const FixedPoint& p) {
    return os << std::fixed << std::setprecision(2) << p.GetRepr() / (double)100;
}